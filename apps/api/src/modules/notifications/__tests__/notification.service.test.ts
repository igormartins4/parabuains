...
function makePrefsRepo(overrides: Partial<INotificationPreferencesRepository> = {}): INotificationPreferencesRepository {
  return {
...
function makeLogRepo(overrides: Partial<INotificationLogRepository> = {}): INotificationLogRepository {
  return {
...
function makeUserRepo(users: Record<string, { id: string; email: string; displayName: string; username: string }>): IUserLookup {
  return {
...
const profileUser = { id: 'profile-uuid', email: 'profile@test.com', displayName: 'Usuário Perfil', username: 'profile' };
const authorUser = { id: 'author-uuid', email: 'author@test.com', displayName: 'Autor Teste', username: 'autor' };
const requesterUser = { id: 'requester-uuid', email: 'req@test.com', displayName: 'Solicitante', username: 'solicitante' };
const actorUser = { id: 'actor-uuid', email: 'actor@test.com', displayName: 'Ator', username: 'ator' };
...
describe('NotificationService.handleWallMessagePosted', () => {
  it('envia email quando preferencia email esta habilitada', async () => {
    const emailPrefs: NotificationPreference[] = [
      { id: 'p1', userId: 'profile-uuid', channel: 'email', daysBefore: [], enabled: true, updatedAt: new Date() },
    ];
  ...
  it('NAO envia email quando preferencia email esta desabilitada', async () => {
    const disabledPrefs: NotificationPreference[] = [
      { id: 'p1', userId: 'profile-uuid', channel: 'email', daysBefore: [], enabled: false, updatedAt: new Date() },
    ];
    const emailTransport = makeEmailTransport();
...
describe('NotificationService.handleFriendshipAccepted', () => {
  it('envia email quando preferencia email esta habilitada', async () => {
    const emailPrefs: NotificationPreference[] = [
      { id: 'p1', userId: 'requester-uuid', channel: 'email', daysBefore: [], enabled: true, updatedAt: new Date() },
    ];
    const emailTransport = makeEmailTransport();
  ...
  it('NAO envia email quando preferencia email esta desabilitada', async () => {
    const disabledPrefs: NotificationPreference[] = [
      { id: 'p1', userId: 'requester-uuid', channel: 'email', daysBefore: [], enabled: false, updatedAt: new Date() },
    ];
    const emailTransport = makeEmailTransport();
